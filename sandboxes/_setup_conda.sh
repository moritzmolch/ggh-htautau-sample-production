...
setup_conda () {
    ...
    local install_flag_file="${PROD_CONDA_BASE}/install"
    ...
    if [[ "${install_conda}" == "true" ]]; then
        # recreate conda base directory if it already exists 
        if [[ -d "${PROD_CONDA_BASE}" ]]; then
            rm -rf "${PROD_CONDA_BASE}" || return "$?"
            mkdir -p "${PROD_CONDA_BASE}" || return "$?"
        fi
